...
async function makeApp(): Promise<FastifyInstance> {
  ...
  // Re-implement the route here using the same handler shape as server.ts so
  // we can test it in isolation. (In Task 3 step 3 we update server.ts to
  // match.)
    ...
    async (req, reply) => {
      ...
      try {
        await deployMod.deploy(
          { apkPath: apk_path, crashWaitMs: crash_wait_ms, onEvent: writeEvent },
          stayAwake,
          logcat,
        );
      } catch (err) {
